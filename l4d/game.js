...
function toggleSound(){sound=!sound;if(!sound){stopGameOverSound();stopDialogue();stopPistolSounds();stopWitchCry();stopZoeyScream();}ui.sound.textContent=sound?'M SOUND ON':'M SOUND OFF';DeadwalkInput.refreshMenuActions();initAudio();}
...
function sync(dt=1/60){player=run.players[controlledIndex];zombies=run.zombies;bullets=run.bullets;particles=run.effects;time=run.time;kills=run.kills;heads=run.heads;const live=cameraActors(),xs=live.map(p=>p.x),ys=live.map(p=>p.y),spanX=xs.length?Math.max(...xs)-Math.min(...xs):0,spanY=ys.length?Math.max(...ys)-Math.min(...ys):0,targetZoom=portraitMode()?clamp(W/225,1.65,1.9):clamp(Math.min(1,(W-140)/(spanX+160),Math.max(100,H-365)/(spanY+180)),.12,1),target=portraitMode()?portraitTarget(targetZoom):live.length?(Math.min(...xs)+Math.max(...xs))/2:player.x,mean=ys.length?(Math.min(...ys)+Math.max(...ys))/2:player.y,elapsed=clamp(dt,0,.05),follow=cameraReady?1-Math.exp(-(portraitMode()?8:6)*elapsed):1;

viewZoom+=(targetZoom-viewZoom)*(cameraReady?1-Math.exp(-(targetZoom<viewZoom?8:3)*elapsed):1);cameraMeanY=cameraMeanY===null?mean:cameraMeanY+(mean-cameraMeanY)*follow;if(portraitMode())cameraMeanY=Math.max(cameraMeanY,player.y-(H-238-portraitAnchor())/viewZoom);const targetCamera=clamp(target-W/viewZoom*.5,0,Math.max(0,5800-W/viewZoom));camera=clamp(camera+(targetCamera-camera)*follow,0,Math.max(0,5800-W/viewZoom));cameraReady=true;run.spawnView={left:camera,right:camera+W/viewZoom,zoom:viewZoom,portrait:portraitMode()};updateHud();}

function updateMobileHud(){if(!mobileMode)return;const context=run.context(player),use=document.querySelector('[data-touch=use]');use.textContent=context?.kind==='revive'?'RESCUE':context?.kind==='safeDoor'?(run.safeDoor.open?'CLOSE':'OPEN'):'USE';use.disabled=!context||!['revive','safeDoor','ladder','gate','ammo'].includes(context.kind)||player.down||player.dead||!!player.grab;use.hidden=use.disabled;for(const [action,count,label] of [['grenade',player.grenades,'GRENADE'],['pills',player.healthItems.pills,'PILLS'],['adrenaline',player.healthItems.adrenaline,'ADREN'],['kit',player.medicine,'HEAL']]){for(const button of document.querySelectorAll('[data-touch='+action+']')){button.textContent=label+' '+count;button.hidden=!count;button.disabled=!count||player.down||player.dead||!!player.grab||player.healthUseTime>0;}}document.getElementById('switch-character').textContent=player.name.toUpperCase()+' Â· SWITCH';if(player.prompt&&!player.beingHealedBy)ui.context.textContent=context?.kind==='revive'?'HOLD RESCUE TO REVIVE':context?.kind==='safeDoor'?'TAP '+use.textContent+' TO USE DOOR':'';if(!player.down&&!player.dead&&!player.grab&&!player.healthUseTime&&!player.reload)ui['weapon-state'].textContent=player.weapon==='pipebomb'?'GRENADE READY':'AUTO RELOAD Â· LIGHT '+(player.flashlight?'ON':'OFF');updatePortraitHud();}
