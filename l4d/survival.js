...
const DW = (() => {
  ...
  class Run {
    ...
    enemyDamage(p,n,attacker=null){if(n<=0)return;if(attacker?.id&&attacker.hp>0){p.lastAttackerId=attacker.id;p.lastAttackerUntil=this.time+3;if(p.ai&&!p.dead){const point=enemyHitboxes(attacker).body;p.angle=aimAt(p,point.x,point.y);p.facing=Math.sign(attacker.x-p.x)||p.facing||1;p.brain.mode='retaliate';p.brain.target=attacker.x;p.brain.nextDecision=Math.max(p.brain.nextDecision||0,this.time+1);}}if(this.romero)this.damage(p,p.down?n:p.hp+(p.tempHp||0));else this.damage(p,n*this.enemyDamageMultiplier);}
    ...
    specialEnemy(z,dt){if(!['hunter','smoker','boomer'].includes(z.role))return false;z.specialCooldown=Math.max(0,z.specialCooldown-dt);
      ...
      if(z.grabbing){const p=this.players.find(p=>p.grab===z.id);if(!p||p.dead){this.release(z);return true}if(z.role==='hunter'){z.x=p.x;z.y=p.y;this.enemyDamage(p,26*dt,z);}else if(z.roofSmoker){z.hoistTime=(z.hoistTime||0)+dt;if(z.hoistTime>.1){const dx=z.x-p.x,nx=p.x+Math.sign(dx)*Math.min(Math.max(0,Math.abs(dx)-45),600*dt);if(this.clearLane(p.x,p.y-38,nx,p.y-38))p.x=nx;const target=z.y+20;p.y=Math.max(target,p.y-600*dt);p.smokerLiftHeight=Math.max(p.smokerLiftHeight||0,p.smokerGroundY-p.y);p.onGround=false;p.vy=0;}this.enemyDamage(p,9*dt,z);}else{const dx=z.x-p.x;if(Math.abs(dx)>65){const nx=p.x+Math.sign(dx)*Math.min(Math.abs(dx)-65,90*dt);if(this.clearLane(p.x,p.y-38,nx,z.y-38)&&Math.abs(p.y-z.y)<65)p.x=nx;}this.enemyDamage(p,9*dt,z);}return true;}
    ...
    knockSurvivor(p,source,damage=45,balanced=false,attacker=null){if(p.dead)return;for(const z of this.zombies)if(z.id===p.grab)this.release(z);attacker=attacker||this.zombies.find(z=>z.hp>0&&z.role==='tank'&&Math.abs(z.x-source)<2)||(!balanced&&damage===35?this.zombies.filter(z=>z.hp>0&&z.role==='tank').sort((a,b)=>Math.abs(a.x-p.x)-Math.abs(b.x-p.x))[0]:null);if(balanced)this.damage(p,damage);else this.enemyDamage(p,damage,attacker);if(p.dead)return;this.drop(p);p.sewerClimbing=false;p.ladderRegrabAt=this.time+1.5;p.knockVX=Math.sign(p.x-source||1)*950;p.knockTime=1.3;p.vy=-430;p.onGround=false;p.reload=0;this.events.push({type:'tankImpact',x:p.x,y:p.y});}
    ...
    enemyMoveStep(z,dt){if(this.romero){z.speed=18;z.horde=false;}dt*=this.enemyTimeScale??1;z.pileStumble=Math.max(0,(z.pileStumble||0)-dt);z.attack-=dt;z.stagger=Math.max(0,z.stagger-dt);if(z.hp<=0)return;if(z.liftBileVent&&z.dropping){z.vy+=1000*dt;z.y+=z.vy*dt;if(z.y>=z.dropFloor){z.y=z.dropFloor;z.vy=0;z.dropping=false;}return;}if(z.role==='tank'){this.tankStep(z,dt);return;}if(z.role==='witch'){this.witchStep(z,dt);return;}if(this.ledgeLeapStep(z,dt))return;if(this.specialEnemy(z,dt))return;if(!this.romero&&this.hordeActive>0&&!['tank','witch','hunter','smoker','boomer'].includes(z.role)){z.horde=true;z.speed=Math.max(z.speed,310+(z.id%8)*5);}const focus=this.romeroFocusTarget(),bomb=focus?null:this.pipeBombs.at(-1);const alive=this.players.filter(p=>!p.dead),coated=alive.filter(p=>p.goo>0),live=coated.length?coated:alive;let p=focus||(this.romero?live.reduce((a,b)=>!a||Math.abs(b.x-z.x)+Math.abs(b.y-z.y)<Math.abs(a.x-z.x)+Math.abs(a.y-z.y)?b:a,null):this.commonTarget(z,live));if(!p)return;if(this.romero){z.target=p.id;z.rushing=!bomb&&Math.hypot(p.x-z.x,p.y-z.y)<=160;z.speed=z.rushing?300:18;}if(bomb){p={x:bomb.x,y:bomb.y+5};z.windup=0;z.grabbing=0;for(const a of this.players)if(a.grab===z.id)a.grab=0;}
      ...
      if(z.grabbing>0){const victim=this.players.find(a=>a.grab===z.id);z.grabbing-=dt;if(z.grabbing<=0){if(victim)victim.grab=0;z.attack=2}else if(victim)this.enemyDamage(victim,dt*5,z);return}

      if(z.windup>0){z.windup-=dt;if(z.windup<=0&&Math.abs(p.x-z.x)<50&&Math.abs(p.y-z.y)<65&&this.clearLane(z.x,z.y-35,p.x,p.y-35)){if(z.role==='grabber'&&!p.down&&!this.players.some(a=>a.grab)){p.grab=z.id;z.grabbing=3;this.message('Grabbed! F / LB shoves free; partner can shoot or shove.')}else this.enemyDamage(p,z.role==='runner'&&this.difficulty==='expert'?12:8,z);z.attack=1.2}return}
    ...
    witchStep(z,dt){z.moving=false;if(!z.awake){z.x=z.restX??z.x;z.y=z.restY??z.y;const contact=this.players.find(p=>!p.dead&&!p.down&&Math.abs(p.x-z.x)<30&&Math.abs(p.y-z.y)<65);if(contact){this.provokeWitch(z,contact);return;}const lit=this.players.slice().sort((a,b)=>Math.hypot(a.x-z.x,a.y-z.y)-Math.hypot(b.x-z.x,b.y-z.y)).find(p=>{if(!p.flashlight||p.dead||p.down||p.grab||p.carry||p.weapon==='pipebomb')return false;const ray=this.laserTrace(p,550,true);if(!ray)return false;const origin=weaponOrigin(p),dx=z.x-origin.x,dy=z.y-40-origin.y,along=dx*Math.cos(origin.angle)+dy*Math.sin(origin.angle),side=Math.abs(-dx*Math.sin(origin.angle)+dy*Math.cos(origin.angle));return along>0&&along<550&&side<10+along*.24&&Math.hypot(ray.endX-ray.x,ray.endY-ray.y)>=along-32&&this.clearLane(origin.x,origin.y,z.x,z.y-40);});const before=z.agitation||0,closeLight=lit&&Math.hypot(lit.x-z.x,lit.y-z.y)<=180,limit=closeLight?1:lit?.45:0;z.agitation=before<limit?Math.min(limit,before+dt/(closeLight?4:6)):Math.max(limit,before-dt/2.5);if(lit)z.facing=Math.sign(lit.x-z.x)||1;for(const threshold of [.2,.65])if(before<threshold&&z.agitation>=threshold)this.events.push({type:'witchAgitated',witch:z.id,x:z.x,y:z.y,stage:threshold});if(closeLight&&z.agitation>=1){this.provokeWitch(z,lit);return;}const nearby=this.players.find(p=>!p.dead&&Math.hypot(p.x-z.x,p.y-z.y)<380);z.cryTimer=(z.cryTimer||0)-dt;if(nearby&&z.cryTimer<=0&&z.agitation<.1){z.cryTimer=6.2;this.events.push({type:'witchCry',witch:z.id,x:z.x,y:z.y});}return;}
      ...
      if(Math.abs(p.x-z.x)<48&&Math.abs(p.y-z.y)<65&&this.clearLane(z.x,z.y-35,p.x,p.y-35)&&z.attack<=0){z.attack=.85;z.slashFlash=.2;if(!p.down){this.enemyDamage(p,p.hp+(p.tempHp||0),z);z.slashes=0;this.message(`${p.name} incapacitated by the Witch! Kill her before she finishes them!`);}else{z.slashes++;this.enemyDamage(p,20,z);if(z.slashes>=5&&!p.dead)this.die(p);}this.events.push({type:'witchSlash',player:p.id,x:p.x,y:p.y});}
...
const baseBotInput=DW.Run.prototype.botInput;
DW.Run.prototype.botInput=function(bot){
  const attacker=this.zombies.find(z=>z.id===bot.lastAttackerId&&z.hp>0&&(z.role!=='witch'||z.awake));
  if(bot.ai&&attacker&&!bot.dead&&!bot.grab&&this.time<(bot.lastAttackerUntil||0)){
    const point=this.botAimPoint(bot,attacker),angle=DW.aimAt(bot,point.x,point.y),origin=DW.weaponOrigin(bot,angle),near=Math.abs(attacker.x-bot.x)<82&&Math.abs(attacker.y-bot.y)<70,canFire=!bot.carry&&!bot.healthHeld&&!bot.healthUseTime&&!bot.beingHealedBy&&!bot.healingOther&&!bot.knockTime&&this.botAlarmSafe(bot,angle)&&this.clearLane(origin.x,origin.y,point.x,point.y);
    bot.brain.mode='retaliate';bot.brain.target=attacker.x;bot.facing=Math.sign(attacker.x-bot.x)||bot.facing||1;
    return {angle,fire:canFire,shove:near&&!bot.down&&!bot.carry&&!bot.knockTime&&!['witch','tank'].includes(attacker.role)&&bot.shoveCooldown<=0&&bot.shove<=0&&!bot.prev.shove,reload:!bot.down&&bot.mag===0&&!bot.reload,heal:false,use:false,crouch:false};
  }
  return baseBotInput.call(this,bot);
};
